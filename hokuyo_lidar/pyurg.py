# Original code: https://github.com/HiroakiMatsuda/pyurg
# modified for Jetson Nano & python3
# 04.11.2021 - Pawel Kapusta, Piotr Duch
...
class Urg(object):
        ...
        def open_port(self, port = 'COM1', baudrate = 115200, timeout = 0.1):
        ...
        def set_port(self, baudrate = 115200, timeout = 0.1):
                self.myserial.baudrate = baudrate
                self.myserial.timeout = timeout
...
# Setting the operating mode
        ...
        def set_scip2(self):
                ...
                read = self.myserial.read(8)[:-1].decode('utf-8')
        ...
        def turn_on_laser(self, string = ''):
                ...
                raw = self.myserial.read(8 + len(string)).decode('utf-8')
                return raw[-5:-2]
...
# Getting Distance Data
        def request_md(self, amin, amax, unit = 1, interval = 0, num = 1, string = ''):
                ...
                head = self.myserial.read(21 + len(string)).decode('utf-8')
                status = head[-5:-3]
        ...
        def request_gd(self, amin, amax, unit = 1, string = ''):
                ...
                head = self.myserial.read(18 + len(string)).decode('utf-8')
                status = head[-5:-3]
                calc_sum = self._calc_sum(status)
                self._check_sum(head[-3], calc_sum)       
        ...
        def get_distance(self):
                ...
                # Read Echo
                echo = self.myserial.read(echo_length).decode('utf-8')
                # Read and Check status
                status = self.myserial.read(4)[:-2].decode('utf-8')
                self.check_status(status)
                # Read, Check and Encode Timestamp
                raw_timestamp = self.myserial.read(4).decode('utf-8')
                calc_sum = self._calc_sum(raw_timestamp)
                check_sum = self.myserial.read(2).decode('utf-8')
                ...
                for i in range(line_num):
                        raw = self.myserial.read(66).decode('utf-8')
                ...
                if odd_byte != 0:
                        raw = self.myserial.read(odd_byte + 2).decode('utf-8')
                        calc_sum = self._calc_sum(raw[:-2])
                        self._check_sum(raw[-2], calc_sum)
                        data.append(raw[:-2])
                # Read last LF
                self.myserial.read(1).decode('utf-8')
        ...
        def get_distance_and_intensity(self):
                ...
                self.check_status(status)
                # Read, Check and Encode Timestamp
                raw_timestamp = self.myserial.read(6).decode('utf-8')
...
# The following functions are provided for use in Urg class
        def _write_command(self, command):
                self.myserial.flushOutput()
                self.myserial.flushInput()
                self.myserial.write(bytes(''.join(command), 'utf-8'))
        ...
        def _read_information(self, string, info_num):
                head = self.myserial.read(7 + len(string)).decode('utf-8')
                info = {}
                ...
                for i in range(info_num):
                        ...
                        read = ''
                        while read != '\n':
                                read = (self.myserial.read(1)).decode('utf-8')
                                raw += read
                        print(raw)
                        if raw[0] != '\n':
